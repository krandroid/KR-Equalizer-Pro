...
class EqService : Service() {
    ...
    override fun onCreate() {
        ...
        try {
            // INI DIA YANG BIKIN GLOBAL - SESSION 0
            eqInstance = Equalizer(0, 0).apply { enabled = true }
            loadLastPreset()
            Toast.makeText(this, "KR EQ Global Aktif", Toast.LENGTH_SHORT).show()
        } catch (e: Exception) {
            Toast.makeText(this, "Diblokir: ${e.message}", Toast.LENGTH_LONG).show()
            stopSelf()
        }
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.action == ACTION_UPDATE_EQ) {
    ...
    private fun createNotification(): Notification {
        ...
        .setContentTitle("KR Equalizer Global Aktif")
        .setContentText("Bass JBL ON - Ketuk untuk setting")
